...
function openTarget(snapshot, item = snapshot.items[0]) {
  return {
...
test("shared version-two fixtures match the extension request schema", async () => {
  ...
  const activationRequest = JSON.parse(
    await readFile(new URL("../../Fixtures/ChromeNativeMessaging/activate-request-v2.json", import.meta.url)),
  );
  const openRequest = JSON.parse(
    await readFile(new URL("../../Fixtures/ChromeNativeMessaging/open-youtube-request-v2.json", import.meta.url)),
  );
...
test("YouTube route and extractor-result validation fail closed on untrusted data", async () => {
  ...
  assert.equal(valid.observationWasTruncated, false);
...
test("YouTube feed read returns only bounded typed items and explicit completeness", async () => {
  const stub = chromeStub([youtubeTab()], {
    extractedFeed: extractedFeed({eligibleItemCount: 3, incompleteItemCount: 1}),
  });
  ...
  assert.equal(response.youTubeFeed.observationWasTruncated, true);
...
test("YouTube open refuses DOM drift, expiry, and permission revocation before mutation", async () => {
  const now = 1000;
  ...
  const changed = await driftHandle({
    version: 2,
    requestID: requestIDs.openVideo,
    operation: "openYouTubeVideo",
    youTubeTarget: openTarget(read.youTubeFeed),
  });
  assert.equal(changed.failureCode, "youTubeFeedChanged");
  assert.equal(driftStub.calls.tabUpdate, 0);
